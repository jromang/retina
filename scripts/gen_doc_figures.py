...
class FigureContext:
    ...
    def survey(
        ...
        *,
        ...
        ``SCNR`` and ``BackgroundNeutralization`` illustrate themselves on a genuine defect
        rather than on one staged for the picture.
        """
        ...
        for band in bands:
            ...
            planes.append((plane - low) / max(high - low, 1e-6))
        data = np.stack(planes, axis=-1) * 0.85 + 0.03
    ...
    # --- writing ------------------------------------------------------------ #
    def save(self, name: str, image) -> Path:
